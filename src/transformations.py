...
def add_fields(df: DataFrame, fields: List[Dict]) -> DataFrame:
    ...
    for field_cfg in fields:
        ...
        if func == "current_timestamp":
            ...
            raise ValueError(f"Unsupported add_fields function: {func!r} for field {name!r}")

    return result


def apply_transformations(dataflow_meta: dict, frames: Dict[str, DataFrame]) -> Dict[str, DataFrame]:
    """
